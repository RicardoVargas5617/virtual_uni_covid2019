	...
	if (!isset($_POST["dni"]))
		{
			...
			#$v = 'admi_ing_subir.php?d='.$_REQUEST['d'].'&nav='.$nav;
			#echo $v;
			#header ($v);
	...
	if ($pasa == 1)
		{
			...
			if ($archivo['size'] <= $max_size && $archivo['size'] > 0){
				########		
				$rutadestino = $zet.'foto_ing_voucher/thumb/';	
				$destino= $zet.'/foto_ing_voucher/'.$dni.'.jpg';
...
<table>
    <tr>
        <td>            
            <a href="admi_ing_sub.php?d=<?php echo $_REQUEST['d']?>&nav=<?php echo $nav?>">
